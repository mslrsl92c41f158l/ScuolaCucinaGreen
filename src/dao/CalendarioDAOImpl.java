...
public class CalendarioDAOImpl implements CalendarioDAO {
	...
	public ArrayList<Edizione> select(int idCaregotia) throws SQLException {
		...
		PreparedStatement ps = conn
				.prepareStatement("SELECT id_edizione FROM calendario join categoria CAT where id_categoria = ?");
	...
	public ArrayList<Edizione> select(java.util.Date da, java.util.Date a) throws SQLException {
		ArrayList<Edizione> editions = new ArrayList<Edizione>();

		PreparedStatement ps = conn.prepareStatement("SELECT * FROM calendario WHERE dataInizio between ? and ?");

		ps.setDate(1, new java.sql.Date(da.getDate()));
		ps.setDate(1, new java.sql.Date(a.getDate()));
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			int idEdizione = rs.getInt("id_Edizione");
			int idCorso = rs.getInt("id_corso");
			...
			Edizione edition = new Edizione(idEdizione, idCorso, dataInizio, durata, aula, docente);
	...
	public ArrayList<Edizione> select(int idCaregotia, boolean future) throws SQLException {
		...
		if (future = false) {
			PreparedStatement ps = conn
					.prepareStatement("SELECT id_edizione, id_corso, dataInizio, durata, aula, docente "
							+ "FROM calendario join catalogo using(id_corso) join categoria using ( id_categoria)"
							+ "where id_categoria = ?");
			...
			PreparedStatement ps = conn.prepareStatement("SELECT id_corso, dataInizio, durata, aula, docente \r\n"
					+ "FROM calendario join catalogo using(id_corso) join categoria using ( id_categoria)\r\n"
					+ "where id_categoria = ? and dataInizio >= curdate()");
	...
	public ArrayList<Edizione> select(boolean future) throws SQLException {
		...
		if (future = false) {
			PreparedStatement ps = conn.prepareStatement("SELECT id_corso, dataInizio, durata, aula, docente \r\n"
					+ "FROM calendario join catalogo using(id_corso);\r\n");
			...
			while (rs.next()) {
				...
				Edizione edition = new Edizione(idCorso, dataInizio, durata, aula, docente);
				editions.add(edition);
			...
			PreparedStatement ps = conn.prepareStatement(
					"SELECT id_corso, dataInizio, durata, aula, docente FROM calendario join catalogo using(id_corso) where dataInizio >= curdate()");
	...
	/*
	 * lettura di tutte le edizioni a cui è iscritto un certo utente, presenti nel
	...
	public ArrayList<Edizione> select(String idUtente, boolean future) throws SQLException {
		...
		if (future = false) {
			PreparedStatement ps = conn.prepareStatement(
					"select id_edizione, id_corso, dataInizio, durata, aula, docente from calendario join iscritti using(id_edizione) where id_utente = ?");
			...
			PreparedStatement ps = conn.prepareStatement(
					"select id_edizione, id_corso, dataInizio, durata, aula, docente from calendario join iscritti using(id_edizione) where id_utente = ? and dataInizio >= curdate()");
